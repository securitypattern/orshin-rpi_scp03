 ...
 *
 ******************************************************************************/
static bool_t phNxpEseProto7816_SendRawFrame(void *conn_ctx, uint32_t data_len,
                                             uint8_t *p_data);
static bool_t phNxpEseProto7816_GetRawFrame(void *conn_ctx, uint32_t *data_len,
                                            uint8_t **pp_data);
static uint16_t phNxpEseProto7816_ComputeCRC(unsigned char *p_buff,
                                             uint32_t offset, uint32_t length);
static bool_t phNxpEseProto7816_CheckCRC(uint32_t data_len, uint8_t *p_data);
static bool_t phNxpEseProto7816_SendSFrame(void *conn_ctx,
                                           sFrameInfo_t sFrameData);
static bool_t phNxpEseProto7816_SendIframe(void *conn_ctx,
                                           iFrameInfo_t iFrameData);
static bool_t phNxpEseProto7816_sendRframe(void *conn_ctx,
                                           rFrameTypes_t rFrameType);
static bool_t phNxpEseProto7816_SetFirstIframeContxt(void);
static bool_t phNxpEseProto7816_SetNextIframeContxt(void);
static bool_t phNxpEseProro7816_SaveRxframeData(uint8_t *p_data,
                                                uint32_t data_len);
...
static bool_t phNxpEseProto7816_CheckCRC(uint32_t data_len, uint8_t *p_data) {
  ...
  recv_crc =
      p_data[data_len - 2] << 8 | p_data[data_len - 1];  // combine 2 byte CRC
 ...
 *
 ******************************************************************************/
uint8_t getMaxSupportedSendIFrameSize(void) { return IFSC_SIZE_SEND; }
...
static bool_t phNxpEseProto7816_sendRframe(void *conn_ctx,
...
#endif
  uint16_t calc_crc = 0;
  iFrameInfo_t *pRx_lastRcvdIframeInfo =
      &phNxpEseProto7816_3_Var.phNxpEseRx_Cntx.lastRcvdIframeInfo;
  rFrameInfo_t *pNextTx_RframeInfo =
      &phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.RframeInfo;
  ...
  recv_ack[PH_PROPTO_7816_PCB_OFFSET] |=
      ((pRx_lastRcvdIframeInfo->seqNo ^ 1) << 4);
  T_SMLOG_D("%s recv_ack[PH_PROPTO_7816_PCB_OFFSET]:0x%x ", __FUNCTION__,
            recv_ack[PH_PROPTO_7816_PCB_OFFSET]);
  calc_crc =
      phNxpEseProto7816_ComputeCRC(recv_ack, 0x00, (sizeof(recv_ack) - 2));
...
static bool_t phNxpEseProto7816_SendIframe(void *conn_ctx,
  ...
  uint8_t pcb_byte = 0;
  uint16_t calc_crc = 0;
  iFrameInfo_t *pNextTx_IframeInfo =
      &phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo;

  if (0 == iFrameData.sendDataLen) {
    T_SMLOG_E("%s Line: [%d] I frame Len is 0, INVALID ", __FUNCTION__,
              __LINE__);
    return FALSE;
  }
  ...
  if (iFrameData.dataOffset != 0) {
    T_SMLOG_I(
        "%s Line: [%d] - dataOffset cannot be greater than zero, as max appdu "
        "buffer size is 128 ",
        __FUNCTION__, __LINE__);
    return FALSE;
  }

  if ((iFrameData.sendDataLen + PH_PROPTO_7816_INF_BYTE_OFFSET) >
      MAX_APDU_BUFFER) {
    T_SMLOG_I("%s Line: [%d] - APDU buffer is not long enough. ", __FUNCTION__,
              __LINE__);
    return FALSE;
  }

  memmove(p_framebuff + PH_PROPTO_7816_INF_BYTE_OFFSET, p_framebuff,
  ...
  p_framebuff[PH_PROPTO_7816_LEN_UPPER_OFFSET] =
      (((uint16_t)iFrameData.sendDataLen) >> 8 & 0xff);
  p_framebuff[PH_PROPTO_7816_LEN_LOWER_OFFSET] =
      (((uint16_t)iFrameData.sendDataLen) & 0xff);
#endif
  /* store I frame */
...
static bool_t phNxpEseProto7816_SetFirstIframeContxt(void) {
  ...
  iFrameInfo_t *pNextTx_IframeInfo =
      &phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.IframeInfo;
  iFrameInfo_t *pLastTx_IframeInfo =
  ...
  if (pNextTx_IframeInfo->totalDataLen > pNextTx_IframeInfo->maxDataLen) {
    ...
    pNextTx_IframeInfo->totalDataLen =
        pNextTx_IframeInfo->totalDataLen - pNextTx_IframeInfo->maxDataLen;
  } else {
    pNextTx_IframeInfo->sendDataLen = pNextTx_IframeInfo->totalDataLen;
    pNextTx_IframeInfo->isChained = FALSE;
  }
  T_SMLOG_D("I-Frame Data Len: %d Seq. no:%d ", pNextTx_IframeInfo->sendDataLen,
            pNextTx_IframeInfo->seqNo);
  return TRUE;
}
...
static bool_t phNxpEseProto7816_SetNextIframeContxt(void) {
  ...
  pNextTx_IframeInfo->seqNo = (uint8_t)(pLastTx_IframeInfo->seqNo ^ 1);
  if ((UINT8_MAX - pLastTx_IframeInfo->dataOffset) <
      pLastTx_IframeInfo->maxDataLen) {
    return FALSE;
  }
  pNextTx_IframeInfo->dataOffset =
      pLastTx_IframeInfo->dataOffset + pLastTx_IframeInfo->maxDataLen;
...
static bool_t phNxpEseProro7816_SaveRxframeData(uint8_t *p_data,
  ...
  T_SMLOG_D("Data[0]=0x%x len=%ld Data[%ld]=0x%x Data[%ld]=0x%x", p_data[0],
            data_len, data_len - 1, p_data[data_len - 2], data_len,
            p_data[data_len - 1]);
  if (pRx_EseCntx->pRsp != NULL) {
    ...
    if ((pRx_EseCntx->responseBytesRcvd + data_len) > pRx_EseCntx->pRsp->len) {
      // LOG_W("Need '%ld' bytes. Got '%ld' to copy.",
      // (size_t)(pRx_EseCntx->responseBytesRcvd + data_len),
      // pRx_EseCntx->pRsp->len);
      return FALSE;
    }
    phNxpEse_memcpy(
        (pRx_EseCntx->pRsp->p_data + pRx_EseCntx->responseBytesRcvd), p_data,
        data_len);
 ...
 ******************************************************************************/
static bool_t phNxpEseProto7816_RecoverySteps(void) {
  sFrameInfo_t *pRx_lastRcvdSframeInfo =
      &phNxpEseProto7816_3_Var.phNxpEseRx_Cntx.lastRcvdSframeInfo;
  sFrameInfo_t *pNextTx_SframeInfo =
...
static void phNxpEseProto7816_DecodeSFrameData(uint8_t *p_data) {
...
#elif defined(T1oI2C_GP1_0)
  /* current GP implementation support max payload of 0x00FE, so considering
   * lower offset */
  ...
  while (maxSframeLen > frameOffset) {
    ...
    T_SMLOG_D("%s frameoffset=%d value=0x%x ", __FUNCTION__, frameOffset,
              p_data[frameOffset]);
 ...
 *
 ******************************************************************************/
static bool_t phNxpEseProto7816_DecodeFrame(uint8_t *p_data,
                                            uint32_t data_len) {
  bool_t status = TRUE;
  uint8_t pcb;
  ...
  iFrameInfo_t *pLastTx_IframeInfo =
      &phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.IframeInfo;
  sFrameInfo_t *pLastTx_SframeInfo =
      &phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.SframeInfo;
  rFrameInfo_t *pRx_lastRcvdRframeInfo =
  ...
  if (0x00 == pcb_bits.msb) /* I-FRAME decoded should come here */
  {
    ...
    {
      T_SMLOG_D("%s I-Frame lastRcvdIframeInfo.seqNo:0x%x ", __FUNCTION__,
                pcb_bits.bit7);
      ...
      if (pcb_bits.bit6) {
        ...
        if (FALSE == phNxpEseProro7816_SaveRxframeData(
                         &p_data[PH_PROPTO_7816_INF_BYTE_OFFSET],
                         data_len - PH_PROTO_7816_INF_FILED)) {
          phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState =
              IDLE_STATE;
    ...
    if ((pcb_bits.lsb == 0x00) && (pcb_bits.bit2 == 0x00)) {
      ...
      if (phNxpEseProto7816_3_Var.recoveryCounter <
        ...
        if (phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.FrameType == IFRAME) {
          ...
          else if ((pRx_lastRcvdRframeInfo->seqNo !=
                    pLastTx_IframeInfo->seqNo) &&
             ...
             (0x01 == pcb_bits.bit7)) /* S-FRAME decoded should come here */
  {
    ...
    switch (frameType) {
      ...
      case WTX_REQ:
        ...
        T_SMLOG_D("%s Wtx_counter value - %lu ", __FUNCTION__,
                  phNxpEseProto7816_3_Var.wtx_counter);
        T_SMLOG_D("%s Wtx_counter wtx_counter_limit - %lu ", __FUNCTION__,
        ...
        if (pLastTx_SframeInfo->sFrameType != WTX_RSP &&
          ...
          if (phNxpEseProto7816_3_Var.wtx_counter ==
            ...
            phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState =
                SEND_S_INTF_RST;
            T_SMLOG_E("%s Interface Reset to eSE wtx count reached!!! ",
                      __FUNCTION__);
#elif defined(T1oI2C_GP1_0)
            phNxpEseProto7816_3_Var.wtx_counter = 0;
            pRx_lastRcvdSframeInfo->sFrameType = SWR_REQ;
            ...
            phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState =
                SEND_S_SWR;
            T_SMLOG_E("%s Software Reset to eSE wtx count reached!!! ",
                      __FUNCTION__);
#endif
          } else {
            sm_sleep(DELAY_ERROR_RECOVERY / 1000);
...
static bool_t phNxpEseProto7816_ProcessResponse(void *conn_ctx) {
  ...
  if (TRUE == status) {
    ...
    T_SMLOG_E("%s phNxpEseProto7816_GetRawFrame failed starting recovery",
              __FUNCTION__);
    if ((SFRAME == phNxpEseProto7816_3_Var.phNxpEseLastTx_Cntx.FrameType) &&
        ((WTX_RSP == pLastTx_SframeInfo->sFrameType) ||
         (RESYNCH_RSP == pLastTx_SframeInfo->sFrameType))) {
      if (phNxpEseProto7816_3_Var.rnack_retry_counter <
          phNxpEseProto7816_3_Var.rnack_retry_limit) {
        ...
        T_SMLOG_E("%s Recovery failed completely, Going to exit ",
                  __FUNCTION__);
        phNxpEseProto7816_3_Var.rnack_retry_counter = PH_PROTO_7816_VALUE_ZERO;
...
static bool_t TransceiveProcess(void *conn_ctx) {
  ...
  while (phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState !=
         IDLE_STATE) {
    T_SMLOG_D("%s nextTransceiveState %x ", __FUNCTION__,
              phNxpEseProto7816_3_Var.phNxpEseProto7816_nextTransceiveState);
...
#error Either T1oI2C_UM11225 or T1oI2C_GP1_0 must be defined.
    ...
    if (TRUE == status) {
      ...
      SMLOG_E("%s Transceive send failed, going to recovery! ", __FUNCTION__);
 ...
 *
 ******************************************************************************/
bool_t phNxpEseProto7816_Transceive(void *conn_ctx, phNxpEse_data *pCmd,
                                    phNxpEse_data *pRsp) {
  bool_t status = FALSE;
  phNxpEseRx_Cntx_t *pRx_EseCntx = &phNxpEseProto7816_3_Var.phNxpEseRx_Cntx;
  iFrameInfo_t *pNextTx_IframeInfo =
 ...
 *
 ******************************************************************************/
bool_t phNxpEseProto7816_Open(void *conn_ctx,
                              phNxpEseProto7816InitParam_t initParam,
  ...
  {
    /*After power ON , initialization state takes 5ms after which slave enters
    active state where slave can exchange data with the master */
    sm_sleep(WAKE_UP_DELAY_MS);

    //INFO: Makes i2c reads
    phNxpEse_clearReadBuffer(conn_ctx);

#if defined(T1oI2C_UM11225)
    /* Interface Reset respond with ATR*/
    SMLOG_I("T1oI2C_UM11225\n");
...
#elif defined(T1oI2C_GP1_0)
    SMLOG_I("T1oI2C_GP1_0\n");

    /* For GP soft reset does not respond with CIP so master should send CIP
     * req. seperatly  */
    status = phNxpEseProto7816_RSync(conn_ctx);
    if (status == TRUE) {
...
bool_t phNxpEseProto7816_Close(void *conn_ctx) {
  sFrameInfo_t *pNextTx_SframeInfo =
      &phNxpEseProto7816_3_Var.phNxpEseNextTx_Cntx.SframeInfo;
  bool_t status = FALSE;
  /*Explicitly Initilising to NULL as the Application layer does not intend to
   * receive a response*/
  phNxpEseRx_Cntx_t *pRx_EseCntx = &phNxpEseProto7816_3_Var.phNxpEseRx_Cntx;
  pRx_EseCntx->pRsp = NULL;
 ...
 *
 * Description      This function is used only for T1oI2C GP to reset just the
 *current interface
 *
 * param[in]        void
 *
