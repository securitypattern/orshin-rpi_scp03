    ...
    uint8_t hasle)
{
    ...
    if(SECLVL_CDEC_RENC_CMAC_RMAC == 0x03 || SECLVL_CDEC_RENC_CMAC_RMAC >= 0x13){ /* Added by me. */
    if (cmdBufLen != 0) {
    ...
    if (apduStatus == SM_OK) {
        ...
        if(SECLVL_CDEC_RENC_CMAC_RMAC >= 0x11){ /* Added by me. */
        ...
        if (tempRspBufLen > (SCP_COMMAND_MAC_SIZE + SCP_GP_SW_LEN)) { // There is data payload in response
            ...
            if(SECLVL_CDEC_RENC_CMAC_RMAC == 0x33){ /* Added by me. */
            ...
            ENSURE_OR_RETURN_ON_ERROR(
              ...
              *pRspBufLen = tempRspBufLen - SCP_COMMAND_MAC_SIZE;
              memcpy(&rspBuf[*pRspBufLen-2], sw, SCP_GP_SW_LEN);
           }
        ...
        if(tempRspBufLen > SCP_GP_SW_LEN){ // There is data payload in the response.
        *pRspBufLen = tempRspBufLen;
        memcpy(&rspBuf[*pRspBufLen-2], sw, SCP_GP_SW_LEN);
        SMLOG_MAU8_D("Decrypted Data ==>", rspBuf, *pRspBufLen);
       }
       else{ // There is no data payload in the response.
        memcpy(rspBuf, sw, SCP_GP_SW_LEN);
        *pRspBufLen = SCP_GP_SW_LEN;
        SMLOG_MAU8_D("Decrypted Data ==>", rspBuf, *pRspBufLen);
     }
